...
if [ -z "$BASH_VERSION" ]; then
  commacd::errout "commacd: is only supported for bash"
  return
fi
...
#  ==> readarray -t myarray <<< "$(commacd::split "$PWD")"
...
# falls back to commacd::change_dir()
commacd::change_directory() {
  local dir="$1"
  [[ -z "$dir" ]] && return  # Use cancelled a selection

  if [[ "$PWD" == "$dir" ]]; then
      commacd::errout "commacd: no matches found"
      return 1
...
commacd::choose_match() {
  ...
  for match in "${matches[@]}"; do
    commacd::errout "%*s  %s" "$width" "$((i++))" "$match"
  done
  local selection
  ...
  while true; do
    ...
    if [[ -z "$selection" ]]; then
      ...
      if [[ "${matches[i]}" != "" ]]; then
        ...
        commacd::errout "Invalid selection! %s" "$selection"
      fi
    else
      commacd::errout "Invalid selection! %s" "$selection"
    fi
...
# Utility function used by commacd::forward()
commacd::forward_by_prefix() {
  ...
  readarray -t matches < <(commacd::expand "$(commacd::prefix_glob "$target")")
  filter_pwd
  if [[ ${#matches[@]} == 0 && "$_commacd_nofuzzyfallback" != "on" ]]; then
    readarray -t matches < <(commacd::expand "$(commacd::infix_glob "$target")")
    filter_pwd
  fi
  ...
  if [[ ${#matches[@]} == 0 && "$_commacd_nodeepfallback" != "on" && ! "$target" =~ ^/ ]]; then
    readarray -t matches < <(commacd::expand "$(commacd::prefix_glob "$target" deep)")
    filter_pwd
    if [[ ${#matches[@]} == 0 && "$_commacd_nofuzzyfallback" != "on" ]]; then
      readarray -t matches < <(commacd::expand "**/$(commacd::infix_glob "$target" deep)")
      filter_pwd
    fi
  fi
  case ${#matches[@]} in
...
commacd::forward() {
  local matches dir
  [[ -z "$1" ]] && { commacd::usage "," ; return 1 ; }

  readarray -t matches < <(commacd::forward_by_prefix "$@")
  ...
  case ${#matches[@]} in
    0)
      commacd::errout "No match for '%s'" "$1"
      return 1
      ;;
    1)
      commacd::change_directory "${matches[0]}"
      ;;
    *)
      # https://github.com/shyiko/commacd/issues/12
      # trap 'trap - SIGINT; stty '"$(stty -g)" SIGINT

      dir="$(commacd::choose_match "${matches[@]}")"
...
commacd::backward_projcect_root() {
  local dir="${PWD%/*}"
  while [[ -n "$dir" ]] && ! commacd::marked "$dir"; do
    dir="${dir%/*}"
  done
  ...
  if [[ "$COMMACD_NOTTY" == "on" ]]; then
    ...
    commacd::errout "No project root found"
    return 1
  else
    commacd::change_directory "$dir"
  fi
...
commacd::backward_by_prefix() {
  ...
  if [[ "${target:0:1}" == "/" ]]; then
    ...
    readarray -t parts < <(commacd::split "$PWD")
    num_parts=${#parts[@]}
    if ((num_parts > 1)); then
      for ((idx = num_parts - 2; idx >= 0; --idx)); do
        if [[ "${parts[idx],,}" == /${target,,}* ]]; then
          dir="$(commacd::join '' "${parts[@]:0:idx+1}")"
          break
        fi
      done
  ...
  if [[ "$COMMACD_NOTTY" == "on" ]]; then
    ...
    commacd::errout "No match for '%s'" "$target"
    return 1
  else
    commacd::change_directory "$dir"
  fi
...
commacd::backward_substitute() {
  ...
  readarray -t cwd_parts < <(commacd::split "$PWD")
  num_parts="${#cwd_parts[@]}"
  ...
  if [[ $idx == -1 ]]; then
    commacd::errout "'%s' cannot be matched in the working directory path!" "$target_prefix"
    return 1
  else
    ...
    head="$(commacd::join '' "${head_parts[@]}")"
    readarray -t head_matches < <(commacd::expand "$head")
    # The tail is everything following the replaced path part
    tail_parts=("${cwd_parts[@]:idx+1}")
    tail="$(commacd::join '' "${tail_parts[@]}")"
    ...
    case "${#final_matches[@]}" in
      0)
        commacd::errout "'%s' cannot be matched in same location as '%s'" "$repl_prefix" "${target:1}"
        return 1
        ;;
      1)
        commacd::change_directory "${final_matches[0]}"
        ;;
      *)
        dir="$(commacd::choose_match "${final_matches[@]}")"
        if [[ -z "$dir" ]]; then
          return 0
        else
          commacd::change_directory "$dir"
...
commacd::backward() {
  ...
  [[ $# == 1 && -z "$1" ]] && shift
  case $# in
    0) commacd::backward_projcect_root ;;
    1) commacd::backward_by_prefix "$1" ;;
    2) commacd::backward_substitute "$@" ;;
    *)
      commacd::errout
      commacd::usage ",,"
      return 1
...
commacd::backward_forward_by_prefix() {
  ...
  while [[ -n "$dir" ]]; do
    dir="${dir%/*}"
    readarray -t matches < <(commacd::expand "$dir/$(commacd::prefix_glob "$1")")
    filter_pwd
    if [[ ${#matches[@]} == 0 && "$_commacd_nofuzzyfallback" != "on" ]]; then
      readarray -t matches < <(commacd::expand "$dir/$(commacd::infix_glob "$1")")
      filter_pwd
    fi

    # If no mathces found then try a deep search.
    if [[ ${#matches[@]} == 0 && "$_commacd_nodeepfallback" != "on" ]]; then
      readarray -t matches < <(commacd::expand "$dir/$(commacd::prefix_glob "$1" deep)")
      filter_pwd
      if [[ ${#matches[@]} == 0 && "$_commacd_nofuzzyfallback" != "on" ]]; then
        readarray -t matches < <(commacd::expand "$dir/**/$(commacd::infix_glob "$1" deep)")
      filter_pwd
      fi
    fi
...
commacd::backward_forward() {
  [[ -z "$1" ]] && { commacd::usage ",,," ; return 1 ; }

  local IFS=$'\n'
  local candidates dir
  readarray -t candidates < <(commacd::backward_forward_by_prefix "$1")
  if [[ "$COMMACD_NOTTY" == "on" ]]; then
    printf "%s\n" "${candidates[@]}"
    return
  fi
  ...
  case ${#candidates[@]} in
    ...
    1)
      ...
      ;;
    *)
      dir="$(commacd::choose_match "${candidates[@]}")"
...
commacd::forward_completion() {
  commacd::completion commacd::forward
}

commacd::backward_completion() {
  if [[ ${#COMP_WORDS[@]} -le 2 ]]; then
    commacd::completion commacd::backward
  fi
}

commacd::backward_forward_completion() {
  commacd::completion commacd::backward_forward
}
      ...
      for ((i = 1; i < len; ++i)); do
        ...
        case "$opt" in
          ...
          ( d )
            ...
            ;;
          ( * )
            commacd::errout "Invalid option: '%s'\n" "-$opt"
            $restore_shopt
            return 1
            ;;
        esac
...
alias ,='commacd::main commacd::forward'
alias ,,='commacd::main commacd::backward'
alias ,,,='commacd::main commacd::backward_forward'

if [ -n "$BASH_VERSION" ]; then
  complete -o filenames -F commacd::forward_completion ,
  complete -o filenames -F commacd::backward_completion ,,
  complete -o filenames -F commacd::backward_forward_completion ,,,
fi
